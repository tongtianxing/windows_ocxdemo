...
uses
  ...
  function Base64Encode(InputFile: string):string;
  procedure Base64Decode(InputFile:string; OutputFile: TMemoryStream;FileName:string);
...
begin {Base64 decode}
  ...
  try
    ...
    Ss := TStringStream.Create(InputFile);
    try
      Ms := TMemoryStream.Create;
      try
